...
bool parseOptions(po::variables_map& vm, int argc, char** argv)
{
  ...
  desc.add_options()
    ...
    ("duration,d", po::value<double>()->default_value(1200.0),
     "Maximum difference between newest and oldest buffered message per topic in seconds. Default: 120")
  ...
  {
  double duration_value = vm["duration"].as<double>();
  // ROS_INFO("Duration parameter value: %f", duration_value);
  }
...
bool parseVariablesMap(SnapshotterOptions& opts, po::variables_map const& vm)
{
  if (vm.count("topic"))
  {
    ...
    // ROS_INFO("Topics: ");
    // for (size_t i = 0; i < opts.topics_.size(); ++i)
    // {
    //     ROS_INFO_STREAM(opts.topics_[i]);
    // }
  }
  ROS_INFO("out addTopic.");
  ...
  // ROS_INFO("SnapshotterOptions content:");
  // ROS_INFO("default_memory_limit_: %d", opts.default_memory_limit_);
  // ROS_INFO("default_duration_limit_: %f", opts.default_duration_limit_.toSec());
  // ROS_INFO("default_count_limit_: %d", opts.default_count_limit_);
  // ROS_INFO("opts.topics_.size(): %d", opts.topics_.size());
  // ROS_INFO("all_topics_: %s", opts.all_topics_ ? "true" : "false");
  // ROS_INFO("compression_: %s", opts.compression_.c_str());
